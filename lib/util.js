...
// Node_ENV = true this if statement will run
  if (process.env.NODE_ENV) {
    // fs.appendFile Asynchronously writes data to the log file
    // with the option of allowing integers and strings
    if (status === undefined) {
      /* eslint-disable */
      console.log(noStatus);
      fs.appendFile('logs/log.log', noStat, { mode: 0o666, flag: 'a' }, (err) => {
        if (err) throw err;
      });
    } else {
      console.log(allInfo);
      /* eslint-enable */
      fs.appendFile('logs/log.log', info, { mode: 0o666, flag: 'a' }, (err) => {
        if (err) throw err;
      });
    }
  }
